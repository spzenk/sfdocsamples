...
namespace WebChat.Services
{
    ...
    public class Transition
    {
        ...
        public List<Message> RecieveMessage()
        {
            ...
            Guid roomid = GetGUIDFromQuery( HttpContext.Current.Request.UrlReferrer.Query);
            if (roomid != Guid.Empty)
            {
               tblChatRoom wRoom = ChatManager.GetChatRoom(roomid);
               List<tblMessagePool> messageList = ChatManager.RecieveMessage(wRoom);
               if (messageList != null)
               {
