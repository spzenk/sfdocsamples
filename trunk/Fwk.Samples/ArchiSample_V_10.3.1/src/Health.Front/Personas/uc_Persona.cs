...
namespace Health.Front.Patients
{
    public partial class uc_Persona : UserControl
    {
        ...
        internal void Init()
        {
            ...
            if (Controller.IsInDesignMode()) return;
            EstadoCivilList.AddRange(Controller.EstadoCivilList.Where<ParametroBE>(p => p.IdParametro != (int)CommonValuesEnum.TodosComboBoxValue));
            cmbEstadoCivil.Properties.DataSource = EstadoCivilList;

            cmbTipoDoc.Properties.DataSource = Controller.SearchParametroByParams((int)TipoParametroEnum.TipoDocumento, null);
