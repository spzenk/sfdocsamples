    ...
    public partial class frmPatientAtencion : Fwk.UI.Forms.FormMDIChildBase
    {
        ...
        void EndPopulate(IAsyncResult res)
        {
            ...
            else
            {
                ...
                lblDNI.Text = Controller.CurrentPatient.Persona.NroDocumento;
                lblNombre.Text = Controller.CurrentPatient.Persona.ApellidoNombre;
                lblEdad.Text = Controller.CurrentPatient.Persona.GetLabelEdad();
                //TODO: Poner fecha nacimiento
                if (Controller.CurrentPatient.Persona.Foto == null)
                {
                    this.pictureEdit1.Properties.SizeMode = DevExpress.XtraEditors.Controls.PictureSizeMode.Clip;
                    if (Controller.CurrentPatient.Persona.Sexo.Equals((Int16)Sexo.Masculino))
                        pictureEdit1.Image = Health.Front.Base.Properties.Resources.User_M;
                    else
                        pictureEdit1.Image = Health.Front.Base.Properties.Resources.User_F;
                }
                else
                {
                    this.pictureEdit1.Properties.SizeMode = DevExpress.XtraEditors.Controls.PictureSizeMode.Stretch;
                    pictureEdit1.Image = Fwk.HelperFunctions.TypeFunctions.ConvertByteArrayToImage(Controller.CurrentPatient.Persona.Foto);
                }
                #endregion
                ...
                if (_PatientAllergy == null) return;
                {
                    if (!string.IsNullOrEmpty(_PatientAllergy.ToString()))
                    {
                        ...
                        if (!string.IsNullOrEmpty(_PatientAllergy.OtherAllergy))
                        {
                            ...
                            string[] x = (from p in Controller.AllergiesItemTypesList where listOther.Contains(p.IdParametro.ToString()) select p.Nombre).ToArray<string>();
       ...
       public static frmPatientAtencion GetInstance(Control control)
        ...
        private void frmPatientAtencion_Leave(object sender, EventArgs e)
        {
            if (_Appointment != null)
            {
                ...
                try
                {
                    Controller.UpdateAppoimentStatus(_Appointment);
                }
                catch (Exception ex)
                {
                    this.ExceptionViewer.Show(ex);
