...
{
	/// <summary>
	/// Tests the <see cref="OPMessage"/> class.
	/// </summary>
	[TestFixture]
	public class MessageTest
	{
		...
		public void TestISO88591CharacterSet()
		{
			...
			string output = new OPMessage(Encoding.GetEncoding("ISO-8859-1").GetBytes(input)).MessagePart.GetBodyAsText();
			Assert.AreEqual(expectedOutput, output);
		}
		...
		public void TestWindows1252CharacterSet()
		{
			...
			string output = new OPMessage(Encoding.GetEncoding(1252).GetBytes(input)).MessagePart.GetBodyAsText();
			Assert.AreEqual(expectedOutput, output);
		}
		...
		public void TestContentTypeWithLongName()
		{
			...
			MessageHeader headers = new OPMessage(Encoding.ASCII.GetBytes(messageHeaders), false).Headers;
        ...
        public void TestLoadSimple()
        {
            ...
            string output = OPMessage.Load(new MemoryStream(Encoding.ASCII.GetBytes(input))).MessagePart.GetBodyAsText();
            Assert.AreEqual(expectedOutput, output);
        }
	}
}