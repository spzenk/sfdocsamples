...
namespace OpenPopExamples
{
	...
	public class Examples
	{
		...
		public static List<OPMessage> FetchAllMessages(string hostname, int port, bool useSsl, string username, string password)
		{
			...
			using(Pop3Client client = new Pop3Client())
			{
				...
				List<OPMessage> allMessages = new List<OPMessage>(messageCount);
		...
		public static void FindPlainTextInMessage(OPMessage message)
		{
		...
		public static void FindXmlInMessage(OPMessage message)
		{
		...
		public static void HeadersFromAndSubject(string hostname, int port, bool useSsl, string username, string password, int messageNumber)
		{
			...
			using (Pop3Client client = new Pop3Client())
			{
				...
				if(from.HasValidMailAddress && from.Address.Equals("test@example.com") && "Some subject".Equals(subject))
				{
					...
					OPMessage message = client.GetMessage(messageNumber);
		...
		public static List<OPMessage> FetchUnseenMessages(string hostname, int port, bool useSsl, string username, string password, List<string> seenUids)
		{
			// The client disconnects from the server when being disposed
			using(Pop3Client client = new Pop3Client())
			{
				...
				List<OPMessage> newMessages = new List<OPMessage>();
				...
				for(int i = 0; i<uids.Count; i++)
				{
					...
					if (!seenUids.Contains(currentUidOnServer))
					{
						...
						OPMessage unseenMessage = client.GetMessage(i + 1);
