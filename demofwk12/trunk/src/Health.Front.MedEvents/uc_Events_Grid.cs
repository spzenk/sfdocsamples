...
namespace Health.Front.Events
{
    public partial class uc_Events_Grid : Xtra_UC_Base
    {
        ...
        string Create_Doc()
        {
            ...
            MedicalEventBE wMedicalEvent = ServiceCalls.GetMedicalEvent(_Event.MedicalEventId);

            StringBuilder t = new StringBuilder(template);

            t.Replace("[PROFESIONAL]", String.Concat(wMedicalEvent.NombreApellidoProfesional, "  (", wMedicalEvent.NombreEspesialidad, ")"));
            ...
            {
                pmo = ServiceCalls.PMOFileList.Where(p => p.Code.Equals(wMedicalEvent.PMOQuirurgico.Trim())).FirstOrDefault();
                if (pmo != null)
                {
            ...
            {
                wParam = ServiceCalls.TipoEventoMedicoList.Where(p => p.IdParametro.Equals(Convert.ToInt32(wMedicalEvent.IdTipoConsulta))).FirstOrDefault();
                if (wParam != null)
