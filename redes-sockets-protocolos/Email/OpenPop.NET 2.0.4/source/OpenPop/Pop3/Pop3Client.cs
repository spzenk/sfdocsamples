...
namespace OpenPop.Pop3
{
	...
	public class Pop3Client : Disposable
	{
		...
		/// </summary>
		public Pop3Client(ProxyInfo proxy)
		{
            this._Proxy = proxy;
			SetInitialValues();
		}
		...
		public void Connect(string hostname, int port, bool useSsl, int receiveTimeout, int sendTimeout, RemoteCertificateValidationCallback certificateValidator)
		{
            ...
            try
            {
                ...
                if (_Proxy != null)
                    clientSocket = ConnectViaHTTPProxy(hostname, port);
                else
        ...
        /// <returns></returns>
        TcpClient ConnectViaHTTPProxy(string host, int port,string userName,string password)
        {
            // create an instance of the client proxy factory 
            ProxyClientFactory factory = new ProxyClientFactory();
            ...
            // use the proxy client factory to generically specify the type of proxy to create 
            // the proxy factory method CreateProxyClient returns an IProxyClient object 
            IProxyClient proxy = factory.CreateProxyClient(ProxyType.Http, host,port, userName, password);
		...
		public OPMessage GetMessage(int messageNumber)
		{
			AssertDisposed();
			...
			return new OPMessage(messageContent);
		}
		...
		public MessageHeader GetMessageHeaders(int messageNumber)
		{
			...
			return new OPMessage(messageContent, false).Headers;
		}
